...
import { getChatContactsApi, getChatHistoryApi, sendTextMessageApi, sendFileMessageApi } from '@/lib/api/chatApi';
...
export default function ChatPage() {
   ...
   * 添加系统消息到聊天列表
   */
  const addSystemMessage = useCallback((content: string, contactUserId: string) => {
  ...
  const fetchContacts = async () => {
    ...
    try {
      ...
      if (targetUserId) {
        const targetContactIndex = contactsList.findIndex((c: Contact) => c.userId === targetUserId);
        ...
        if (targetContactIndex > -1) {
          ...
          try {
            ...
            const avatarValue = (userInfo as any).avatarUrl || (userInfo as any).avatar || '';
  ...
  const fetchChatHistory = async (withUserId: string) => {
    ...
    try {
      ...
      console.log('开始获取聊天记录:', { withUserId, currentUserId: user.userId });
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (error.message && error.message.includes('每日消息发送次数已达上限')) {
        systemMessage = '今日消息发送次数已达上限，请明天再试或关注对方后无限制聊天';
      } else if (error.message && error.message.includes('权限')) {
        systemMessage = '您暂时无法向该用户发送消息，请关注对方后再试';
      } else {
        systemMessage = `消息发送失败：${error.message || '网络错误，请稍后重试'}`;
      }
  ...
  const handleSendFile = async (file: File, msgType: MessageType) => {
    ...
    try {
      ...
      const tempMessage: Message = {
        ...
        fromUserId: user!.userId,
        toUserId: currentContact.userId,
        content: `[${msgType === 'image' ? '图片' : msgType === 'video' ? '视频' : msgType === 'voice' ? '语音' : '文件'}] ${file.name}\n正在上传...`,
        msgType: msgType,
        status: 'sending',
        revoked: 0,
        ctime: new Date().toISOString(),
      ...
      if (error.message && error.message.includes('每日消息发送次数已达上限')) {
        ...
        systemMessage = `文件发送失败：${error.message || '网络错误，请稍后重试'}`;
      }
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type.startsWith('image/') || 
        ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp', 'svg', 'ico', 'tiff'].includes(fileExtension)) {
      msgType = 'image';
    } 
    // 视频类型
    else if (file.type.startsWith('video/') || 
             ['mp4', 'avi', 'mov', 'wmv', 'flv', 'webm', 'mkv', 'm4v', '3gp', 'rm', 'rmvb'].includes(fileExtension)) {
      msgType = 'video';
    } 
    // 音频类型
    else if (file.type.startsWith('audio/') || 
             ['mp3', 'wav', 'ogg', 'm4a', 'aac', 'flac', 'wma', 'opus'].includes(fileExtension)) {
      msgType = 'voice';
    }
    // 其他文件类型保持为 'file'
   ...
   * 选择联系人
   */
  const handleSelectContact = async (contact: Contact, event?: React.MouseEvent) => {
    // 完全阻止默认行为和事件冒泡
    if (event) {
  ...
  return (
    <div className="h-[calc(100vh-120px)] bg-white dark:bg-dark-primary shadow-xl overflow-hidden flex border border-neutral-200/50 dark:border-zinc-700/50 chat-page-container">
      {/* 左侧联系人列表 */}
              ...
              {contacts.map((contact) => (
                ...
                >
                  ...
                  <div className="relative flex-shrink-0">
                    <img
                      src={contact.avatarUrl || '/images/avatars/default-avatar.svg'}
            ...
            <div className="h-16 bg-white/90 dark:bg-dark-secondary/90 backdrop-blur-sm border-b border-neutral-200 dark:border-zinc-700 flex items-center justify-between px-6 shadow-sm flex-shrink-0">
              <div className="flex items-center space-x-3">
                <div className="relative">
                  <img
                    src={selectedContact.avatarUrl || '/images/avatars/default-avatar.svg'}
              ...
              {messagesLoading ? (
                  ...
                  {[...Array(3)].map((_, index) => (
                    <div key={index} className={`flex ${index % 2 === 0 ? 'justify-end' : 'justify-start'}`}>
                      <div className="flex items-end space-x-2 max-w-xs animate-pulse">
                        {index % 2 !== 0 && <div className="w-8 h-8 bg-neutral-200 dark:bg-neutral-700 rounded-full"></div>}
                ...
                <>
                  {messages.map((message, index) => {
                    ...
                    const isSystemMessage = message.isSystemMessage || message.fromUserId === 'system';
                    ...
                    return (
                      <div
                        key={message.messageId}
                        className={`chat-message-container ${isFromMe ? 'from-me' : 'from-other'} animate-fadeIn`}
                        style={{ animationDelay: `${index * 50}ms` }}
                      >
                        {/* 头像 - 使用统一的chat-avatar类 */}
                        <img
                          src={isFromMe 
                            ? ((user as any).avatar || (user as any).avatarUrl || '/images/avatars/default-avatar.svg')
                            : (selectedContact.avatarUrl || '/images/avatars/default-avatar.svg')
                          }
                          alt="avatar"
                        ...
                        <div className={`chat-bubble ${isFromMe ? 'from-me' : 'from-other'} ${message.msgType === 'image' ? 'image-message' : ''} ${message.status === 'sending' ? 'message-status-sending' : message.status === 'sent' ? 'message-status-sent' : ''}`}>
                          {/* 消息内容 */}
                            ...
                            {message.msgType === 'text' ? (
                                ...
                                {(() => {
                                  ...
                                  for (let i = 0; i < lines.length; i++) {
                                    ...
                                    if (line.startsWith('http://') || line.startsWith('https://')) {
                                      ...
                                      // 提取文件名
                                      const match = line.match(/\[图片\]\s*(.+)/);
                                      if (match) {
                                        fileName = match[1];
                                      }
                                    } else if (line && !line.includes('[图片]') && !line.includes('正在上传')) {
                                      description = line;
                                    }
                                  }
                                      ...
                                      {imageUrl ? (
                                        <div className="relative rounded-lg overflow-hidden max-w-sm cursor-pointer group">
                                            ...
                                            onClick={() => {
                                              ...
                                              if (newWindow) {
                                                newWindow.document.write(`
                                                  <html>
                                                    <head><title>图片查看 - ${fileName || '图片'}</title></head>
                                          ...
                                          <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-10 transition-all duration-200 flex items-center justify-center">
                                            <svg className="w-8 h-8 text-white opacity-0 group-hover:opacity-80 transition-opacity duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              ...
                              (() => {
                                ...
                                for (let i = 0; i < lines.length; i++) {
                                  ...
                                  } else if (line && !line.includes('正在上传')) {
                                    description = line;
                                  }
                                }
                                
                                // 根据文件扩展名确定图标和样式
                                const getFileIcon = (fileName: string, msgType: string) => {
                                  ...
                                  if (['ppt', 'pptx'].includes(ext)) {
                                    return (
                                      <svg className="w-6 h-6 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 4V2a1 1 0 011-1h8a1 1 0 011 1v2m0 0V3a1 1 0 011 1v1M7 4V3a1 1 0 011-1h8a1 1 0 011 1v1m-9 4h10m-10 4h10m-10 4h6" />
                                      </svg>
                                    );
                                  }
                                  
                                  // 视频类型
                                  if (msgType === 'video' || ['mp4', 'avi', 'mov', 'wmv', 'flv', 'webm', 'mkv'].includes(ext)) {
                                    return (
                                  ...
                                  // 音频类型
                                  if (msgType === 'voice' || ['mp3', 'wav', 'ogg', 'm4a', 'aac', 'flac'].includes(ext)) {
                                    return (
